...
impl DeviceManager {
    ...
    >(
        ...
        // 用来 firecracker 通知 guest 的虚拟中断
        let interrupt = Arc::new(IrqTrigger::new());


        // The device mutex mustn't be locked here otherwise it will deadlock.
        // vm 的 guest_memory 就是 memory region
        // 这里就是包装了一下 device（block device、net device ...），包装成了 MmioTransport
        let device =
    ...
    pub(crate) fn attach_virtio_device<T: 'static + VirtioDevice + MutEventSubscriber + Debug>(
        ...
        // 这里简单说下 mmio 和 pci 这两种方式的区别
        // mmio: firecracker 已经分配好了 MMIO 地址，通过内核参数 cmdline 告诉 guest MMIO 的地址
        // pci: firecracker 只是创建一个 pci 设备，guest 需要通过 PCI 的机制来发现设备，通过读取 BAR 来获取 MMIO 地址

        if self.is_pci_enabled() {
            // TODO Lee P1 学习下 PCI 设备相关的东西，看一下这个方法实现
            self.pci_devices.attach_pci_virtio_device(&kvm_vm, id, device, event_manager)?;
        } else {
